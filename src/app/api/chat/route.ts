...
export async function POST(req: Request) {
  try {
    ...
    if (relevantEmails.length === 0) {
      ...
      const searchTerms = userQuery.toLowerCase().split(/\s+/).filter(term => term.length > 2)
    ...
    const filteredEmails = relevantEmails.filter(email => 
      email.similarity >= SEARCH_CONFIG.SIMILARITY_THRESHOLD
    )
